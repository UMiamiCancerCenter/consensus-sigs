...
cosmic_v3_4_signatures<-readRDS(opt$signatures)
...
# write_csv(rownames_to_column(as.data.frame(max_weights_matrix), var = "sample_name"), file.path(opt$output, "deconstructSigs_cosmic_v3_4_signature.csv"))
write_csv(rownames_to_column(as.data.frame(all_weights_matrix), var = "sample_name"), file.path(opt$output, "deconstructSigs_cosmic_v3_4_all_signatures.csv"))
